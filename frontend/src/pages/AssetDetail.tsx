...
export const AssetDetail: React.FC<AssetDetailProps> = ({ symbol, onBack }) => {
  ...
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      <div className="bg-dark-800 border border-slate-800/80 p-3 md:p-4 rounded-md flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex items-center gap-2.5 md:gap-3 min-w-0">
          <button 
            onClick={onBack}
            className="p-1.5 rounded bg-dark-900 hover:bg-slate-800 text-slate-400 hover:text-white transition-colors border border-slate-800 flex-shrink-0"
            title="Geri Dön"
          >
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 md:gap-2 flex-wrap">
              <h2 className="text-lg md:text-xl font-bold text-white font-mono">{asset.symbol}</h2>
              <span className="text-[9px] md:text-[10px] px-1.5 py-0.5 rounded bg-blue-600/20 text-blue-400 border border-blue-500/30 font-mono">
                {asset.asset_class}
              </span>
              {asset.sector && (
                <span className="text-[11px] md:text-xs text-slate-400 font-sans flex items-center gap-1 truncate">
                  <Building2 className="w-3 h-3 text-slate-500 flex-shrink-0" /> {asset.sector}
                </span>
              )}
            </div>
            <p className="text-xs text-slate-300 font-sans mt-0.5 truncate">{asset.name}</p>
          </div>
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end flex-wrap">
          {asset.symbol.startsWith('BIST') && (
            <div className="flex items-center bg-dark-900 p-0.5 rounded border border-slate-800 text-[10px] md:text-xs">
              <button
          ...
          <button
            ...
            className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs font-mono font-bold transition-colors ${
              addedToPortfolio
