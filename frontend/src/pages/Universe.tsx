...
export const Universe: React.FC<UniverseProps> = ({ onSelectAsset }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 bg-dark-800 p-2.5 md:p-3 rounded-md border border-slate-800/80">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            className="w-full bg-dark-900 border border-slate-700/80 rounded pl-9 pr-4 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
          />
        </div>

        {/* Varlık Sınıfı Hızlı Filtreleri */}
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar py-0.5">
          {[
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded text-[10px] md:text-xs font-mono font-medium transition-colors whitespace-nowrap flex-shrink-0 ${
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs min-w-[650px] md:min-w-full">
            <thead className="bg-dark-900/80 text-slate-500 font-mono text-[10px] border-b border-slate-800 sticky top-0">
              <tr>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium cursor-pointer hover:text-slate-300" onClick={() => handleSort('symbol')}>
                  <div className="flex items-center gap-1">
                    <span>SEMBOL</span>
                    <ArrowUpDown className="w-3 h-3" />
                  </div>
                </th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium cursor-pointer hover:text-slate-300" onClick={() => handleSort('name')}>
                  ŞİRKET ADI
                </th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium cursor-pointer hover:text-slate-300" onClick={() => handleSort('sector')}>
                  SEKTÖR
                </th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium cursor-pointer hover:text-slate-300" onClick={() => handleSort('composite_score')}>
                  <div className="flex items-center gap-1">
                    <span>SKOR (/10)</span>
                    <ArrowUpDown className="w-3 h-3" />
                  </div>
                </th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium">TEMEL NOT</th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium">SİNYAL</th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium">GÜVEN</th>
                <th className="py-2.5 px-3 md:py-3 md:px-4 font-medium text-right">DETAY</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60 font-mono text-xs">
              {loading ? (
                ...
                filteredAssets.map((asset) => {
                  ...
                  >
                    <td className="py-2.5 px-3 md:px-4 font-bold text-slate-200 group-hover:text-blue-400">
                      {asset.symbol}
                    </td>
                    <td className="py-2.5 px-3 md:px-4 text-slate-300 font-sans font-medium max-w-[140px] md:max-w-none truncate">
                      {asset.name}
                    </td>
                    <td className="py-2.5 px-3 md:px-4 text-slate-400 font-sans text-[11px] max-w-[100px] md:max-w-none truncate">
                      {asset.sector || '—'}
                    </td>
                    <td className="py-2.5 px-3 md:px-4">
                      {asset.composite_score ? (
                        <div className="flex items-center gap-1.5 md:gap-2">
                          ...
                          <div className="hidden sm:block w-12 md:w-16 h-1.5 bg-dark-900 rounded-full overflow-hidden border border-slate-800">
                            <div
                    ...
                    <td className="py-2.5 px-3 md:px-4 text-right">
                      <button className="inline-flex items-center gap-1 text-slate-500 group-hover:text-blue-400 text-xs">
                        <span className="hidden xs:inline">İncele</span>
